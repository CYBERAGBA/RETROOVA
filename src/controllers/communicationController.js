...
class CommunicationController {
    ...
    send = async (req, res) => {
        ...
        if (!recipient || recipient.id === req.session.userId || blocked || (itemId && (!item || item.user_id !== recipient.id)) || !message || message.trim().length < 2) return res.redirect('/messages?error=Identifiant public, annonce ou message invalide ou bloqué');
        await this.model.sendMessage({ id: uuidv4(), notificationId: uuidv4(), senderId: req.session.userId, senderPublicId: sender?.public_id, receiverId: recipient.id, itemId, subject, message: message.trim() });
        res.redirect('/messages?message=Message envoyé');
    };
    ...
    notifications = async (req, res) => { const notifications = await this.model.getNotifications(req.session.userId); res.render('pages/notifications', { title: 'Notifications', notifications }); };
